...
unsigned int pci_list_pos = 0;
char pci_list_str[PCI_LIST_SIZE];
...
grub_lspci_iter (int bus, int dev, int func, grub_pci_id_t pciid)
{
  ...
  if(pci_list_pos > (PCI_LIST_SIZE - 20))
  {
    grub_printf("PCI list full!\n");
    return -1;
  }

  //grub_printf ("%02x:%02x.%x %04x:%04x.%d", 0, dev, func, pciid >> 16, pciid & 0xFFFF, func);
  //grub_printf ("%x:%x.%x %x:%x.%d", 0, dev, func, pciid >> 16, pciid & 0xFFFF, func);
  pci_list_pos += sprintf(pci_list_str + pci_list_pos, "%x:%x.%x %x:%x.%d", 0, dev, func, pciid >> 16, pciid & 0xFFFF, func);
  ...
  //grub_printf (" %s\n", sclass);
  pci_list_pos += sprintf (pci_list_str + pci_list_pos, " %s\n", sclass);

  return 0;
}

int bootjacker_fillpci (void)
{
